...
class PPO:
    ...
    def train(self, env: gym.Env, num_iterations=1000, num_episodes_per=10, num_updates_per=10, validate_per=50):
        ...
        for iteration in tqdm(range(num_iterations), desc="Training"):
            ...
            if iteration % validate_per == 0:
                logging.info(f"Iteration {iteration}: Training loss: {actor_loss.item()}, Critic loss: {critic_loss.item()}")
                mean = self.validate(env, num_episodes=10)
                
                # Early stopping condition
                if mean > best_mean:
                    best_mean = mean
                    logging.info(f"Iteration {iteration}: New best mean reward: {mean}")
    ...
    def validate(self, env: gym.Env, num_episodes=10):
        ...
        for _ in range(num_episodes):
            ...
            episode_reward = 0
            while not done:
                action, _ = self.select_action(torch.FloatTensor(state))
                next_state, reward, terminated, truncated, _ = env.step(action)
        ...
        logging.info(f"Validation: Mean reward: {mean_reward}, Std reward: {std_reward}")
...
if __name__ == "__main__":
    ...
    ppo_agent.train(env, num_iterations=1000, num_episodes_per=10, num_updates_per=10)
